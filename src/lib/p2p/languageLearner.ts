...
export class LanguageLearner {
  ...
  /** Export transitions as a serializable object */
  exportTransitions(): Record<string, { nextTokens: Record<string, number>; totalWeight: number }> {
    ...
    for (const [context, entry] of this.transitions) {
      const nextTokens: Record<string, number> = {};
      for (const [token, weight] of entry.nextTokens) {
        nextTokens[token] = weight;
      }
      out[context] = { nextTokens, totalWeight: entry.totalWeight };
    }
    return out;
  }
  ...
  mergeTransitions(external: Record<string, { nextTokens: Record<string, number>; totalWeight: number }>): void {
    if (!external) return;
    const DECAY = 0.8; // slight decay for incoming transitions to preserve local learning
    const MAX_CONTEXTS = 8000;
    const MAX_TOKENS_PER_CONTEXT = 200;

    for (const [context, incoming] of Object.entries(external)) {
      if (!incoming || !incoming.nextTokens) continue;
      let entry = this.transitions.get(context);
      if (!entry) {
        entry = { nextTokens: new Map(), totalWeight: 0 };
        this.transitions.set(context, entry);
      ...
      for (const [token, weight] of Object.entries(incoming.nextTokens)) {
        const decayed = weight * DECAY;
        const current = entry.nextTokens.get(token) ?? 0;
        entry.nextTokens.set(token, current + decayed);
        entry.totalWeight += decayed;
      }

      // Cap tokens per context
      if (entry.nextTokens.size > MAX_TOKENS_PER_CONTEXT) {
        const sorted = Array.from(entry.nextTokens.entries())
          .sort((a, b) => b[1] - a[1])
          .slice(0, MAX_TOKENS_PER_CONTEXT);
        entry.nextTokens = new Map(sorted);
        entry.totalWeight = sorted.reduce((s, [, w]) => s + w, 0);
      }
    }
    ...
    if (this.transitions.size > MAX_CONTEXTS) {
      ...
      for (const [k, v] of sorted) this.transitions.set(k, v);
    }
  }

  /** Export merged phrases */
  exportMergedPhrases(): string[] {
    return Array.from(this.mergedPhrases);
  }

  /** Import merged phrases and compile into transitions */
  mergePhrases(phrases: string[]): void {
    if (!phrases) return;
    for (const phrase of phrases) {
      if (phrase && !this.mergedPhrases.has(phrase)) {
        this.mergedPhrases.add(phrase);
      }
    }
  }

  /** Get top tokens overlapping with a set of theme words */
  getTopTokensOverlapping(themeWords: string[], n = 5): TokenStats[] {
    const themeSet = new Set(themeWords.map(w => w.toLowerCase()));
    const overlapping = Array.from(this.vocabulary.entries())
      .filter(([token]) => themeSet.has(token))
      .map(([token, frequency]) => ({ token, frequency }))
      .sort((a, b) => b.frequency - a.frequency)
      .slice(0, n);
    return overlapping;
